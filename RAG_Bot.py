...
- Contexts for the 3 sources: '<pirl_context>{pirl_context}</pirl_context>
<scene7_context>{scene7_context}<scene7_context>
<archive_context>{archive_context}<archive_context>'
...
- Direct to Allied Technical Support if unable to solve or find a response or if conversation is going nowhere
...
<pirl_context>{pirl_context}</pirl_context>
<scene7_context>{scene7_context}<scene7_context>
<archive_context>{archive_context}<archive_context>  
...
def ask_open_ai(query, chat_history):
    ...
    for result in results_main:
        #if result["filepath"].startswith('data/AlliedTechPubs/'):
        results_pirl.append({
        ...
        # elif result["filepath"].startswith('data/Scene7/'):
        ...
        #     })
        # elif result["filepath"].startswith('data/AlliedTechSupportDocs/'):
